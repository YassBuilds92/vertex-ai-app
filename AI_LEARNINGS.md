...
- [Icon Generation Error] -> Corrigé en passant de `imagen-3.0` à `gemini-2.5-flash-image` dans `/api/generate-image`. Si l'erreur 500 persiste sur Vercel, vérifier les quotas Vertex AI ou les filtres de sécurité. L'API renvoie désormais un champ `details` pour faciliter le débugging côté frontend.
...
- [Interruption Streaming / Troncature] -> Si la réponse s'arrête en plein milieu d'une phrase sans erreur visible (carré rouge disparu), c'est souvent dû à la limite `maxOutputTokens` (par défaut 2048). **Solution** : Augmenter `maxOutputTokens` à 8192 dans `useStore.ts` (frontend) et `api/index.ts` (backend). Ajouter un log pour `finishReason` dans le backend pour différencier `MAX_TOKENS` des filtres `SAFETY`.
