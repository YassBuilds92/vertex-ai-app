...
- [Erreur JSON / SyntaxError <] -> Le frontend reçoit du HTML au lieu de JSON (ex: redirection login, erreur Vercel 504, 404 sur route API manquante). Solution : 1. Rendre `authMiddleware` plus permissif pour TOUTES les routes commençant par `/api`. 2. Ajouter un handler 404 spécifique pour `/api/*`. 3. Ajouter un handler d'erreur global `app.use((err, req, res, next) => ...)` qui renvoie du JSON pour les routes API.
