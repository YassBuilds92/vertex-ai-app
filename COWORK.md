...
2. Tester specifiquement les demandes creatives documentees (ex: terme d'argot + ecriture rap) pour verifier que Cowork fait bien `report_progress` -> `web_search` contextualises -> `web_fetch` -> redaction, sans partir trop vite sur une generation finale.
