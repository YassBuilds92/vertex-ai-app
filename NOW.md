...
## Objectif actuel
- Revalider dans la vraie app que `Nasheed Studio` garde sa nouvelle version epuree et premium avec de vraies donnees, sans retomber dans une surface trop textuelle ou trop proche d'un chat.

## Blocage actuel
- Pas de blocage code immediat.
- La surface `Nasheed Studio` est maintenant simplifiee et revalidee sur harness local, mais pas encore rejouee avec une app persistee reelle dans le shell complet authentifie.
...
- Ouvrir `Cowork Apps` dans la vraie app, cliquer sur une app musicale/Nasheed reelle et verifier que:
  ...
  - la scene garde bien sa hierarchie sobre (header court, hero compact, rails clairs) avec les vraies donnees du blueprint
...
- Observer si la colonne `Sorties recentes` et le hero central doivent etre encore plus compacts sur desktop de faible hauteur.
...
- Le rendu est valide sur harness local, mais doit encore etre ressenti dans le shell complet en session reelle avec des briefs plus longs que la fixture locale.
