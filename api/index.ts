  ...
  try {
    ...
    const localTools: LocalToolDefinition[] = [
      ...
      },
      {
        ...
        description: "Genere une image locale avec le modele image de ton choix, l'ecrit dans '/tmp/' et renvoie le chemin du fichier. Aliases acceptes pour `model`: `Nano Banana` -> `gemini-2.5-flash-image`, `Nano Banana 2` -> `gemini-3.1-flash-image-preview`, `Nano Banana Pro` -> `gemini-3-pro-image-preview`, `GPT Image 2` -> `gpt-image-2`. Utile pour une cover, une illustration, un visuel ou une vignette avant publication via 'release_file'.",
        parameters: {
          type: "object",
          properties: {
