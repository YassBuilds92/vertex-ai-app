...
function sanitizeAttachment(attachment: Attachment): Attachment {
  return {
    ...
    generationMeta: attachment.generationMeta,
  };
}
...
function mergeRunMeta(current?: Partial<RunMeta>, incoming?: Partial<RunMeta>): RunMeta {
  return {
    ...
    workerCallsCount: Math.max(Number(current?.workerCallsCount || 0), Number(incoming?.workerCallsCount || 0)),
    workerMsTotal: Math.max(Number(current?.workerMsTotal || 0), Number(incoming?.workerMsTotal || 0)),
    ...
    embeddingCount: Math.max(Number(current?.embeddingCount || 0), Number(incoming?.embeddingCount || 0)),
    embeddingTokens: Math.max(Number(current?.embeddingTokens || 0), Number(incoming?.embeddingTokens || 0)),
    vectorSearches: Math.max(Number(current?.vectorSearches || 0), Number(incoming?.vectorSearches || 0)),
    pythonExecutions: Math.max(Number(current?.pythonExecutions || 0), Number(incoming?.pythonExecutions || 0)),
    gitOps: Math.max(Number(current?.gitOps || 0), Number(incoming?.gitOps || 0)),
    browserOps: Math.max(Number(current?.browserOps || 0), Number(incoming?.browserOps || 0)),
...
export function loadCoworkSessionSnapshotEntries(userId: string): Array<{ sessionId: string; messages: Message[] }> {
  if (!userId) return [];
