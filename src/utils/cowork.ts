...
function mergeRunMeta(current?: Partial<RunMeta>, incoming?: Partial<RunMeta>): RunMeta {
  return {
    ...
    blockerCount: Number(incoming?.blockerCount ?? current?.blockerCount ?? 0),
    ...
    executionMode: (incoming?.executionMode || current?.executionMode || 'research_loop') as RunMeta['executionMode'],
    publicPhase: String(incoming?.publicPhase || current?.publicPhase || incoming?.phase || current?.phase || 'analysis'),
    phase: String(incoming?.phase || current?.phase || 'analysis'),
    completionScore: Number(incoming?.completionScore ?? current?.completionScore ?? 0),
    modelCompletionScore: Number(incoming?.modelCompletionScore ?? current?.modelCompletionScore ?? 0),
